...
from graphrag.Core.Common.Utils import truncate_list_by_token_size
from graphrag.Core.Index.TFIDFStore import TFIDFIndex
...
class EntityRetriever(BaseRetriever):
    ...
    async def _find_relevant_entities_by_relationships_agent(self, query: str, total_entity_relation_list: list[dict],
        ...
        # ✅
        try:
            from graphrag.Core.Prompt.TogPrompt import score_entity_candidates_prompt
            total_candidates = []
            total_scores = []
            total_relations = []
