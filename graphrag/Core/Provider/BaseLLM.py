...
from graphrag.Core.Common.Constants import LLM_API_TIMEOUT, USE_CONFIG_TIMEOUT
from graphrag.Core.Common.Logger import logger
from graphrag.Core.Schema.Message import Message
from graphrag.Core.Common.Utils import log_and_reraise
from graphrag.Core.Common.CostManager import CostManager, Costs
...
class BaseLLM(ABC):
    ...
    @abstractmethod
    async def acompletion(self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT, max_tokens: int = None):
    ...
    @abstractmethod
    async def _achat_completion_stream(self, messages: list[dict], timeout: int = USE_CONFIG_TIMEOUT, max_tokens: int = None) -> str:
