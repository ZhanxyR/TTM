...
from graphrag.Core.Common.Utils import (
    clean_str,
    split_string_by_multi_markers,
    is_float_regex
)
from graphrag.Core.Schema.ChunkSchema import TextChunk
from graphrag.Core.Schema.Message import Message
from graphrag.Core.Prompt import GraphPrompt
from graphrag.Core.Schema.EntityRelation import Entity, Relationship
from graphrag.Core.Common.Constants import (
    DEFAULT_RECORD_DELIMITER,
    DEFAULT_COMPLETION_DELIMITER,
    DEFAULT_TUPLE_DELIMITER,
    DEFAULT_ENTITY_TYPES
)
from graphrag.Core.Common.Memory import Memory
from graphrag.Core.Storage.NetworkXStorage import NetworkXStorage
...
class RKGraph(BaseGraph):
    ...
    async def _build_graph(self, chunk_list: List[Any]):
        try:
            ...
            pbar = tqdm(total=total, desc="Building Graph")

            async def wrapped_task(chunk):
                result = await self._extract_entity_relationship(chunk)
                pbar.update(1)
                return result

            tasks = [wrapped_task(chunk) for chunk in chunk_list]
