...
from graphrag.Core.Common.ContextMixin import ContextMixin
from graphrag.Core.Schema.RetrieverContext import RetrieverContext
from graphrag.Core.Common.TimeStatistic import TimeStatistic
from graphrag.Core.Graph import get_graph
from graphrag.Core.Index import get_index, get_index_config
from graphrag.Core.Query import get_query
from graphrag.Core.Storage.NameSpace import Workspace
from graphrag.Core.Community.ClusterFactory import get_community
from graphrag.Core.Storage.PickleBlobStorage import PickleBlobStorage
...
#
    ...
    async def insert(self, docs: Union[str, list[Any]]):
        ...
        if self.config.use_entities_vdb:
            ...
            await self.entities_vdb.build_index(await self.graph.nodes_data(),node_metadata, force=self.config.graph.force)
        ...
        if self.config.use_relations_vdb:
            ...
            if not edge_metadata:
                ...
                return
            await self.relations_vdb.build_index(await self.graph.edges_data(), edge_metadata, force=self.config.graph.force)
        ...
        if self.config.use_subgraphs_vdb:
            ...
            await self.subgraphs_vdb.build_index(await self.graph.subgraphs_data(), subgraph_metadata, force=self.config.graph.force)
        ...
        if self.config.graph.use_community:
            ...
            await self.community.generate_community_report(self.graph, self.config.graph.force)
        self._update_costs_info("Index Building")

        await self._build_retriever_context()
    
    # async def query_chunks(self, query):
