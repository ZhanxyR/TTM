...
def parse():
    ...
    parser.add_argument('-i', '--input', type=str, default='evaluations/ablation_results', help='The directory of inputs.')
...
if __name__ == '__main__':
    ...
    for subject in subjects:
        ...
        roles = os.listdir(os.path.join(args.input, subject))
        roles = [r for r in roles if os.path.isdir(os.path.join(args.input, subject, r))]

        for role in roles:
            print(f'Evaluating {subject}/{role}...')

            if os.path.exists(os.path.join(args.input, subject, role, 'evaluation_records.json')):
                continue

            files = glob(os.path.join(args.input, subject, role, '*.txt'))
            ...
            for i in range(len(keys)):
                for j in range(i+1, len(keys)):
                    ...
                    prompt = get_prompt_template(content_A, content_B, role, subject)
                    ...
                    # inverse
                    prompt = get_prompt_template(content_B, content_A, role, subject)
                    response = model.invoke(prompt)
            ...
            with open(os.path.join(args.input, subject, role, 'evaluation_records.json'), 'w', encoding='utf-8') as f:
    ...
    # record
    for subject in subjects:
        print(f'Recording {subject}...')

        roles = os.listdir(os.path.join(args.input, subject))
        roles = [r for r in roles if os.path.isdir(os.path.join(args.input, subject, r))]

        for role in roles:
            with open(os.path.join(args.input, subject, role, 'evaluation_records.json'), 'r', encoding='utf-8') as f:
                records = json.load(f)
            for key in records:
                content = records[key]
                key = key.replace('Memory-checked', 'Memory_checked')
                ...
                C_P_scores.setdefault(key.split('-')[0], []).append(scores['Consistency of Persona'][0])
                C_P_scores.setdefault(key.split('-')[1], []).append(scores['Consistency of Persona'][1])

                A_K_scores.setdefault(key.split('-')[0], []).append(scores['Accuracy of Knowledge'][0])
                A_K_scores.setdefault(key.split('-')[1], []).append(scores['Accuracy of Knowledge'][1])

                Q_C_scores.setdefault(key.split('-')[0], []).append(scores['Quality of Conversation'][0])
                Q_C_scores.setdefault(key.split('-')[1], []).append(scores['Quality of Conversation'][1])
