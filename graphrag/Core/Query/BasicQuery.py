...
from graphrag.Core.Common.Logger import logger
from graphrag.Core.Common.Constants import Retriever
from graphrag.Core.Common.Utils import list_to_quoted_csv_string, truncate_list_by_token_size, combine_contexts, process_combine_contexts
from graphrag.Core.Prompt import QueryPrompt
...
class BasicQuery(BaseQuery):
    ...
    chunks only get 
    '''
    async def _retrieve_relevant_contexts_global_keywords_chunks_only(self, query, top_k):
        query = await self.extract_query_keywords(query, "high")
        if query == '' or query == None : return [], None, None
        edge_datas = await self._retriever.retrieve_relevant_content(seed=query, type=Retriever.RELATION, mode="vdb")
        if edge_datas == None : return [], None, None
        ...
        for i, t in enumerate(use_text_units):
            text_units_section_list.append(t)
        return text_units_section_list, query, edge_datas
    
    async def _retrieve_relevant_contexts_local_chunks_only(self, query, top_k):
        """
        ...
        for i, t in enumerate(use_text_units):
            text_units_section_list.append(t)
        return text_units_section_list, query, node_datas


    def combine_text(self, text_units_context, hl_text_units_context):
        combined_sources = []
        seen = set()
        for item in text_units_context + hl_text_units_context:
    ...
    chunks only
    '''
    async def _retrieve_relevant_contexts_chunks_only(self, query, text_only, top_k=5):
        text_units_context, hl_text_units_context = None, None
        ...
        if self.config.enable_local or self.config.enable_hybrid_query:
            text_units_context, key_node, node_datas = await self._retrieve_relevant_contexts_local_chunks_only(query, top_k=top_k)
        
        if self.config.enable_hybrid_query:
            hl_text_units_context, key_edge, edge_datas = await self._retrieve_relevant_contexts_global_keywords_chunks_only(query, top_k=top_k)

        text_ruturn = self.combine_text(text_units_context[:top_k], hl_text_units_context[:top_k])

        if text_only:
            text_return_list = [["id", "content"]]
            for i, t in enumerate(text_ruturn):
                text_return_list.append([i, t])
            text_ruturn = list_to_quoted_csv_string(text_return_list)

        return text_ruturn, key_node, node_datas, key_edge, edge_datas
    ...
    async def generation_qa(self, query, context):
        ...
        if self.config.community_information and self.config.use_global_query:
            ...
            print(self.config.community_information, self.use_global_query, self.config.use_keywords, self.config.enable_local)
