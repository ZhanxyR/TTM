...
from graphrag.Core.Retriever.MixRetriever import MixRetriever
from typing import Any
from graphrag.Core.Prompt import GraphPrompt, QueryPrompt
from graphrag.Core.Common.Utils import clean_str, prase_json_from_response, truncate_list_by_token_size, \
...
class BaseQuery(ABC):
    ...
    @abstractmethod
    async def _retrieve_relevant_contexts_chunks_only(self, **kwargs):
        pass
    ...
    '''
    add query for chunks only
    query : str -> response : chunks
    '''
    async def query_chunks_only(self, query, text_only, top_k):
        context, key_node, node_datas, key_edge, edge_datas = await self._retrieve_relevant_contexts_chunks_only(query=query, text_only=text_only, top_k=top_k)
        return context, key_node, node_datas, key_edge, edge_datas
